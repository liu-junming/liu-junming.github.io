...
window.toggleAuthors = function(element) {
    var authorContainer = element.parentElement;
    var hiddenAuthors = authorContainer.querySelector('.author-hidden');
    
    if (hiddenAuthors.classList.contains('show')) {
        hiddenAuthors.classList.remove('show');
        element.textContent = ' ...more';
    } else {
        hiddenAuthors.classList.add('show');
        element.textContent = ' ...less';
    }
};

// Pixel About-Me Interface Handler
document.addEventListener('DOMContentLoaded', function() {
    console.log('Setting up pixel about-me interface...');
    
    const pixelAboutSection = document.getElementById('pixel-about-me');
    const mastheadWrapper = document.getElementById('masthead-wrapper');
    const pageAboutBackground = document.querySelector('.page-about_background');
    const startGameButton = document.getElementById('startGameButton');
    
    if (!pixelAboutSection) {
    ...
    // 初始状态：显示像素风界面，隐藏正常内容
    if (mastheadWrapper) mastheadWrapper.style.display = 'none';
    if (pageAboutBackground) pageAboutBackground.style.display = 'none';
    pixelAboutSection.style.display = 'block';
    ...
    if (startGameButton) {
        startGameButton.addEventListener('click', function(e) {
            ...
            // 切换到正常内容
            pixelAboutSection.style.transition = 'opacity 0.5s ease-out';
            pixelAboutSection.style.opacity = '0';
            
            setTimeout(function() {
                pixelAboutSection.style.display = 'none';
                if (mastheadWrapper) {
                    mastheadWrapper.style.display = 'block';
                    mastheadWrapper.style.opacity = '0';
                    mastheadWrapper.style.transition = 'opacity 0.4s ease-in';
                    setTimeout(function() {
                        mastheadWrapper.style.opacity = '1';
                    }, 50);
                }
                if (pageAboutBackground) {
                    pageAboutBackground.style.display = 'block';
                }
                window.scrollTo(0, 0);
    ...
    let resetTimeout;
    const SCROLL_THRESHOLD = -150; // 需要向上滚动的累积阈值
    const RESET_DELAY = 500; // 重置延迟（毫秒）
    
    window.addEventListener('wheel', function(e) {
        if (isTransitioning) return; // 防止重复触发
        
        // 如果像素风界面可见且向下滚动
        if (pixelAboutSection.style.display !== 'none') {
            if (e.deltaY > 0) { // 向下滚动
                console.log('Scroll down detected - switching to normal content');
                e.preventDefault();
                isTransitioning = true;
                scrollAccumulator = 0; // 重置累积器
                
                // 切换到正常内容
                pixelAboutSection.style.transition = 'opacity 0.4s ease-out';
                pixelAboutSection.style.opacity = '0';
                ...
                setTimeout(function() {
                    ...
                    if (mastheadWrapper) {
                        mastheadWrapper.style.display = 'block';
                        mastheadWrapper.style.opacity = '0';
                        mastheadWrapper.style.transition = 'opacity 0.3s ease-in';
                        setTimeout(function() {
                            mastheadWrapper.style.opacity = '1';
                        }, 50);
                    }
        ...
        // 如果正常内容可见且在顶部向上滚动
        else if (pixelAboutSection.style.display === 'none' && window.pageYOffset === 0) {
            if (e.deltaY < 0) { // 向上滚动
                // 累积向上滚动的距离
                ...
                resetTimeout = setTimeout(function() {
                    scrollAccumulator = 0;
                    console.log('Scroll accumulator reset');
                }, RESET_DELAY);
                
                console.log('Scroll accumulating:', scrollAccumulator, '/', SCROLL_THRESHOLD);
                
                // 只有当累积滚动超过阈值时才触发切换
                if (scrollAccumulator <= SCROLL_THRESHOLD) {
                    console.log('Scroll threshold reached - switching to pixel interface');
                    e.preventDefault();
                    isTransitioning = true;
                    scrollAccumulator = 0; // 重置累积器
                    ...
                    // 切换回像素风界面
                    if (mastheadWrapper) {
                        mastheadWrapper.style.transition = 'opacity 0.3s ease-out';
                        mastheadWrapper.style.opacity = '0';
                        setTimeout(function() {
                            mastheadWrapper.style.display = 'none';
                        }, 300);
                    }
                    
                    setTimeout(function() {
                        if (pageAboutBackground) {
                            pageAboutBackground.style.display = 'none';
                        }
                        pixelAboutSection.style.display = 'block';
                        pixelAboutSection.style.opacity = '0';
                        pixelAboutSection.style.transition = 'opacity 0.4s ease-in';
                        setTimeout(function() {
                            pixelAboutSection.style.opacity = '1';
                            isTransitioning = false;
                        }, 50);
                    }, 300);
                }
